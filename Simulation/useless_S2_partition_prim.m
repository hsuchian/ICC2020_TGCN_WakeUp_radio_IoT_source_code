...
function [partition_result, number_of_subgroups, middle_point] = S2_partition_prim(X, Y, UAVradius)
    ...
    while isempty(remaining_vertex)  ~= 1
        ...
        if (isempty(remaining_edge)  ~= 1)
            min_edge_row_ind_in_RE = 1;                                   % sorted in increasing order
            min_vertex_ind = remaining_edge(min_edge_row_ind_in_RE, 1);   % choose one endpoint as starting point
            ...
            if length(remaining_vertex(1,:)) ~= 1
                %fprintf('Error size of remaining_vertex while facing empty edge set, size = %d\n', length(remaining_vertex(1,:)));
            end
        end
        ...
        if isempty(min_vertex_col_ind_in_RV) || min_vertex_row_index_in_RV ~= 1
            ...
            fprintf('%d   %d\n', min_vertex_ind, min_edge_row_ind_in_RE);
            pause
        end
        ...
        % remove the vertex that is too far away from the current vertex
        tmp_pairwiseD_to_min_vertex = pairwiseD(min_vertex_ind, tmp_remaining_vertex(1,:));
        remove_vertex_ind_in_TRV = find( tmp_pairwiseD_to_min_vertex > 3*UAVradius);
        tmp_remaining_vertex(:, remove_vertex_ind_in_TRV) = [];
            
        %{
        tind = 1;
        L_tmp_vertex = length(tmp_remaining_vertex(1,:));
        while tind  <= L_tmp_vertex && (isempty(tmp_remaining_vertex) == 0)
            if pairwiseD(tmp_remaining_vertex(1,tind), min_vertex_ind) > 3*UAVradius
                tmp_remaining_vertex(:, tind) = [];
                L_tmp_vertex = L_tmp_vertex -1;
            else
                tind = tind +1;
            end
        end
