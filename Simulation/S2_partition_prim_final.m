...
% only adjust the next round finding, the center still use middle point

function [partition_result, number_of_subgroups, middle_point] = S2_partition_prim_final(X, Y, UAVradius)
    ...
    while isempty(remaining_vertex)  ~= 1
        ...
        % find center of current set and calculate the distance of remaining vertex and current set 
        ind_of_current_subgroup = find(set_node(2,:) == set_count);
        cen_x = mean( X(ind_of_current_subgroup) );
        cen_y = mean( Y(ind_of_current_subgroup) );
