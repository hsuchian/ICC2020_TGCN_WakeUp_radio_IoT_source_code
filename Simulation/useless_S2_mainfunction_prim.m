...
function [SuccessProb_prim, TransmissionTime_prim, EnergyConsumption_prim, Times_waked_prim, partition_result_prim, middle_point_prim ] = S2_mainfunction_prim(max_range, num_devices, UAVradius, fscale, pointX, pointY, Length_of_hash)
   ...
   hash_inner(3,:) = hash2;
    ...
    [partition_result_prim, num_subgroups_our, middle_point_prim] = S2_partition_prim(pointX, pointY, UAVradius);
    ...
    for subgroup_index = 1:num_subgroups_our 
        ...
        mid_x = middle_point_prim(1, subgroup_index);
        mid_y = middle_point_prim(2, subgroup_index);
    ...
    Times_waked_prim = mean(times_of_being_waked);
    SuccessProb_prim = mean(success_table);
    % SuccessProb =times_of_being_waked;
    TransmissionTime_prim = mean(Transmission_Time);
    EnergyConsumption_prim = mean(Energy_Consumption);
end
