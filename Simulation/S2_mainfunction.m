...
function [SuccessProb, TransmissionTime, EnergyConsumption, Times_waked, partition_result_our, middle_point ] = S2_mainfunction(max_range, num_devices, UAVradius, fscale, pointX, pointY, Length_of_hash)
   ...
   hash_inner(3,:) = hash2;
    ...
    [partition_result_our, num_subgroups_our, middle_point] = S2_partition(pointX, pointY, UAVradius);
    ...
    for subgroup_index = 1:num_subgroups_our 
        ...
        mid_x = middle_point(1, subgroup_index);
        mid_y = middle_point(2, subgroup_index);
    ...
    Times_waked = mean(times_of_being_waked);
    SuccessProb = mean(success_table);
