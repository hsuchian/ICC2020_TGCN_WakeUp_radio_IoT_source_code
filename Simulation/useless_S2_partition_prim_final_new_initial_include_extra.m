...
function [partition_result, number_of_subgroups, middle_point] = S2_partition_prim_final_new_initial_include_extra(X, Y, UAVradius)
    ...
    while isempty(remaining_vertex)  ~= 1
        ...
        if closet_vertex_last_round_val == int_max   % initial or all the same
            ...
            for ind_5 = 1:length(remaining_vertex(1,:))
                ...
                if norm([X(tmp_vertex_ind) Y(tmp_vertex_ind)]  - [100 100] ) > min_norm
                    min_norm = norm([X(tmp_vertex_ind) Y(tmp_vertex_ind)]  - [100 100] );
            ...
            % choose extremal point as initial 
            min_norm = 0;
            for ind_6 = 1:length(remaining_vertex(1,:))
                tmp_vertex_ind = remaining_vertex(1, ind_6);
                if norm([X(tmp_vertex_ind) Y(tmp_vertex_ind)] - [100 100]) > min_norm
                    min_norm = norm([X(tmp_vertex_ind) Y(tmp_vertex_ind)]  - [100 100] );
                    min_vertex_ind = tmp_vertex_ind;
                end
