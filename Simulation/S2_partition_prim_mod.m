...
function [partition_result, number_of_subgroups, middle_point] = S2_partition_prim_mod(X, Y, UAVradius)
    ...
    set_node = [  1:length(X) ;  -1*ones(1, length(X))  ];             % fix the setting for kruskal algorithm: original set is set to nodes index itself
